...
for (i in unique(sample0@METADATA$POSITION)) {
  ...
  plot(y = gc$gc.1.*(1-exp(-(x+gc$gc.3.)/gc$gc.2.)+gc$gc.4.*x), #+gc$gc.1.*(1-exp(-(x+gc$gc.3.)/gc$gc.2.)+gc$gc.4.*x), 
       x = x,
  ...
  s2 <- sample[[3]]@data[,2]
  t <- sample[[1]]@data[,1]
  par(mar = c(5,20,20,2.1))
  plot(x = t, y = s1, xlab = '', ylab = '', xaxt = 'n', yaxt = 'n', main = '', type = 'l', lwd = 2, lty = 1,
       xaxs = 'i', yaxs = 'i', xlim = c(0,max(t)), ylim = c(0, max(s1,s2)+max(s1,s2)*0.05), cex.lab = 1.5, col = 'black')
  par(new = T)
  plot(x = t, y = s2, xlab = '', ylab = '', xaxt = 'n', yaxt = 'n', main = '', type = 'l', lwd = 2, lty = 1,
       xaxs = 'i', yaxs = 'i', xlim = c(0,max(t)), ylim = c(0, max(s1,s2)+max(s1,s2)*0.05), cex.lab = 1.5, col = 'red')
  axis(2, at = seq(0,max(s1,s2),1000), cex.axis = 1.2)
  mtext(side = 2, 'OSL (cts/0.1 s)', line = 2.2, cex = 1.5)
  axis(3, at = seq(0,max(t),10), cex.axis = 1.2)
...
plot_RadialPlot(results.subset,
...
mtext(text = paste0(caminfo, ' | ', round(camres$rel_OD,1), '%',  aliquot.number),
      adj = 0.5,
      line = 0.5,
      col = 'black')
